...
class PromptController {
    ...
    async getPrompts(req, res, next) {
        try {
            ...
            const { page = '1', pageSize = '20', search, categoryId, tags, isFavorite, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
    ...
    async createPrompt(req, res, next) {
        try {
            const userId = req.user.id;
            const { title, content, description, categoryId, variables = [], metadata = {}, tags = [] } = req.body;
            // 验证必填字段
            if (!title || !content) {
                return res.status(400).json({
                    success: false,
    ...
    async updatePrompt(req, res, next) {
        try {
            const userId = req.user.id;
            const { id } = req.params;
            const { title, content, description, categoryId, variables, metadata, isFavorite, isPublic, tags } = req.body;
            if (!id || isNaN(parseInt(id))) {
                return res.status(400).json({
                    success: false,
                    error: {
